...
class Server {
    ...
    constructor(router, devMode) {
        ...
        // Default to 3000 if a port is not given by Heroku
    ...
    static postResponse() {
        ...
        const isMention = Bot.checkMention(requestMessage);
        const isLex = Bot.checkGroup(requestMessage);

        if(isMention) {
            Bot.sendMention(isLex);
        }
